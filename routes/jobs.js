...
// Search jobs
router.get("/search", (req, res) => jobController.searchJobs(req.body).then(result => res.status(result.statusCode).send(result.response)));

// Change offer status
router.patch("/changestatus/:id", auth.verify, (req, res) => {
    const sessionData = auth.decode(req.headers.authorization);
    jobController.changeOfferStatus(sessionData, req.params.id).then(result => res.status(result.statusCode).send(result.response));
});
